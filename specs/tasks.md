...
- 「新建用户」对话框：username、password、displayName、role。
- 行操作：编辑、重置密码。
...
| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M7-F-01 | AdminRoomsView 列表 | `frontend/.../views/admin/AdminRoomsView.vue` | M7-F-02 | 展示房间 | 未开始 |
| M7-F-02 | admin room API | `frontend/.../api/adminRoom.ts` | M7-C-01 | CRUD+status | 未开始 |
| M7-F-03 | 会议室表单对话框 | `frontend/.../components/admin/RoomFormDialog.vue` | M7-F-01 | 必填 name | 未开始 |

## 5. Controller 任务

| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M7-C-01 | AdminRoomController + RoomController GET /rooms | `backend/.../room/AdminRoomController.java` | M7-S-01 | DELETE 有预约返回 40903 | 未开始 |

## 6. Service 任务

| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M7-S-01 | MeetingRoomAdminService CRUD+status+delete | `backend/.../room/MeetingRoomAdminService.java` | M3-M-01 | 维护中房间 excluded from available | 未开始 |

## 7. Mapper 任务

| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M7-M-01 | MeetingRoomMapper 管理端 CRUD | `backend/.../room/mapper/MeetingRoomMapper.java` | M3-M-01 | 逻辑删除 deleted 标志 | 未开始 |
| M7-M-02 | countFutureBookingsByRoomId | `backend/.../booking/mapper/BookingMapper.java` | M3-M-02 | 删除前计数 | 未开始 |
...
| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M8-F-01 | AdminBookingsView 列表 | `frontend/.../views/admin/AdminBookingsView.vue` | M8-F-02 | 显示全公司数据 | 未开始 |
| M8-F-02 | admin booking API | `frontend/.../api/adminBooking.ts` | M8-C-01 | list/update/cancel | 未开始 |
| M8-F-03 | 编辑预约对话框（时间+房间） | `frontend/.../components/admin/BookingEditDialog.vue` | M8-F-01 | 冲突显示错误 | 未开始 |

## 5. Controller 任务

| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M8-C-01 | GET/PUT/POST cancel admin bookings | `backend/.../booking/AdminBookingController.java` | M8-S-01,M8-S-02 | ADMIN 权限 | 未开始 |

## 6. Service 任务

| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M8-S-01 | AdminBookingQueryService 分页列表 | `backend/.../booking/AdminBookingQueryService.java` | M3-M-02 | 含组织者显示名 | 未开始 |
| M8-S-02 | AdminBookingUpdateService 改时/房+冲突检测 | `backend/.../booking/AdminBookingUpdateService.java` | M3-S-01,M3-S-02 | 重叠失败 40901 | 未开始 |
| M8-S-03 | AdminBookingCancelService | `backend/.../booking/AdminBookingCancelService.java` | M4-S-02 | 取消后通知组织者 | 未开始 |

## 7. Mapper 任务

| ID | 任务 | 主文件 | 前置依赖 | 验收方式 | 状态 |
|----|------|--------|----------|----------|------|
| M8-M-01 | selectAllBookings 分页 | `backend/.../booking/mapper/BookingMapper.java` | M3-M-02 | 参数化分页 | 未开始 |
| M8-M-02 | updateBookingTimeAndRoom | `backend/.../booking/mapper/BookingMapper.java` | M8-M-01 | UPDATE 参数化 | 未开始 |
